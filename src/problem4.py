...
def problem4(number_of_stairs, step_size, starting_point, window):
    ...
    x_val = starting_point.x + step_size
    y_val = starting_point.y - step_size
    total = rg.Line(rg.Point(starting_point.x,starting_point.y),rg.Point(starting_point.x,y_val))
    for _ in range(number_of_stairs):
        total = total + rg.Line(rg.Point(starting_point.x,y_val),rg.Point())
        rg.Line()
